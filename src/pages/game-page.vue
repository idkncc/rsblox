...
onMounted(async () => {
  gameDetails.value = await invoke<GameDetails>(
      "plugin:roblox-api|game_details",
      {universeId: parseInt(route.params.id as string)}
  )
})
    ...
    <div class="game-header grid grid-cols-2 gap-4">
      ...
      <div class="game-info">
        <div class="game-title">
          ...
          <p>by <b>{{ gameDetails.creator.name }}</b></p>
        </div>

        <div class="game-play">
          <button class="play-button" @click="play">
            <svg class="play-button-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
